...
namespace PdfDrive.Views;

// Окно «Спросить у ИИ» — единый узнаваемый элемент SeniorHub (перенос из HomeAccounting/TakingMedications).
public partial class AskAiWindow : Window
{
    ...
    private static readonly WMedia.Brush BrushError = Frozen(211, 47, 47);
    ...
    private static string AiDataDir => Path.Combine(
        Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "PdfDrive");
    ...
    public AskAiWindow()
    {
        ...
        btnAiQuick1.Click += (_, _) => txAiQuestion.Text = Loc.T("ai_q_compress");
        btnAiQuick2.Click += (_, _) => txAiQuestion.Text = Loc.T("ai_q_merge");
        btnAiQuick3.Click += (_, _) => txAiQuestion.Text = Loc.T("ai_q_convert");

        LoadAiSettings();
        ApplyLoc();
        Loc.LanguageChanged += ApplyLoc;
        Closed += (_, _) => Loc.LanguageChanged -= ApplyLoc;
    }

    private void ApplyLoc()
    {
        Title                = Loc.T("ai_title");
        ...
        btnAiQuick1.Content  = Loc.T("ai_qb_compress");
        btnAiQuick2.Content  = Loc.T("ai_qb_merge");
        btnAiQuick3.Content  = Loc.T("ai_qb_convert");
    ...
    private async Task AskClaudeAsync(int idx, string question, string key)
    {
        try
        {
            ...
            SetResponse(idx, doc.RootElement.GetProperty("content")[0].GetProperty("text").GetString() ?? "", null);
        }
    ...
    private async Task AskGeminiAsync(int idx, string question, string key)
    {
        try
        {
            ...
            SetResponse(idx, doc.RootElement.GetProperty("candidates")[0].GetProperty("content")
                .GetProperty("parts")[0].GetProperty("text").GetString() ?? "", null);
        }
        catch (Exception ex) { SetResponse(idx, $"{Loc.T("ai_err")}: {ex.Message}", BrushError); }
    }
    ...
    {
        string dir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "PdfDrive");
